...
def _trial_summary(hparams, examples_path, output_dir):
  ...
  examples_path_summary = tf.summary.text(
      'summary_text_examples_path', tf.constant(examples_path, name='tf_constant_examples_path'),
      collections=[])
  ...
  hparam_summary = tf.summary.text(
      'summary_text_hparams', tf.constant(hparams_table, name='tf_constant_hparams'), collections=[])

  with tf.Session() as sess:
    writer = tf.summary.FileWriter(output_dir+'hparams/', graph=sess.graph)
...
def train(train_dir,
  ...
  with tf.Graph().as_default():
    ...
    # for key, value in transcription_data.items(): print('|', key, '==>>', value )

    # loss, losses, unused_labels, unused_predictions, images = model.get_model(
    #     transcription_data, hparams, is_training=True)
    # print('======================================================================loss, losses, unused_labels, unused_predictions, images')
    ...
    # print('======================================================================update_op', tf.get_collection(tf.GraphKeys.UPDATE_OPS))
    ...
    # # hooks = [tf.train.LoggingTensorHook(logging_dict, every_n_iter=2)]
    # # if num_steps:
