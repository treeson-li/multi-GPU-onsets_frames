...
def sparse_gate_encoder(inputs, params, dtype=None, scope=None, vec_size=256, is_training=True, bidirectional=True):
    with tf.variable_scope(scope, default_name="sparse_gate_encoder", dtype=dtype,
                           values=[inputs]):
        if bidirectional:
            attn_mode = 'full'
        else:
        ...
        x = inputs
        for layer in range(params.encoder_rnn_stack_size):
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("self_attention"):
                    x_att = layers.attention.sparse_multihead_attention(
                        ...
                        keep_prob=1.0 - params.att_dropout,
                        attn_mode=attn_mode,
                    )
                    ...
                    # Gating layer
                    z = layers.nn.linear(tf.concat([x, y], axis=-1), 
                        vec_size*2, True, True, scope="z_project")
                    i, f = tf.split(z, [vec_size, vec_size], axis=-1)
                    y = tf.sigmoid(i) * x + tf.sigmoid(f) * y
                    
                    x = residual_fn(x, y, 1.0 - params.att_dropout)
                    x = layer_process(x, params.layer_postprocess)
...
def sparse_self_attention(inputs, num_units, params, is_training, bidirectional):

    hidden_size = num_units
    
    # input transform
    keep_prob = 1.0 - params.att_dropout
    enc_input = slim.fully_connected(inputs, hidden_size, scope='enc_input')
    enc_input = slim.dropout(enc_input, keep_prob, is_training=is_training, scope='enc_input_dropout')

    # TODO: position embedding
    encoder_input = layers.attention.add_timing_signal(enc_input)
    encoder_input = slim.dropout(encoder_input, keep_prob, is_training=is_training, scope='enc_input_dropout')

    encoder_outputs = sparse_gate_encoder(encoder_input, params, vec_size=num_units, is_training=is_training, bidirectional=bidirectional)
