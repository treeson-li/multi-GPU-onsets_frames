...
def sparse_multihead_attention(queries, memories, num_heads, key_size,
                        value_size, output_size, keep_prob=None, output=True,
                        dtype=None, scope=None, values=None, context=128, attn_mode='full'):
    """ Multi-head scaled-dot-product attention with input/output
        transformations.
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        local_attn = sparse.attention_impl(q, k, v, heads=num_heads, attn_mode=attn_mode, local_attn_ctx=ctx, keep_prob=keep_prob)
        #local_attn = sparse.attention_impl(q, k, v, heads=num_heads, attn_mode='full', local_attn_ctx=ctx, keep_prob=keep_prob)
