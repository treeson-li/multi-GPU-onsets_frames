    ...
    with tf.train.MonitoredSession(session_creator=session_creator) as sess:
      ...
      for unused_i in range(acoustic_data_provider.num_batches):
        ...
        try:
            ...
            print('!! exception at step', unused_i, 'exception:', e)
            continue
        filenames[0] = os.path.split(filenames[0].decode())[1]
        ...
        # for note in sequence_prediction.notes:  # modify error, pitch+=21
        ...
        with tf.gfile.GFile(pianoroll_output_file, mode='w+') as f:
          imageio.imwrite(
              pianoroll_output_file, infer_util.posterior_pianoroll_image(
                  frame_probs, sequence_prediction, labels, overlap=True,
...
def main(unused_argv):
  ...
  if FLAGS.acoustic_checkpoint_filename:
    acoustic_checkpoint = os.path.join(
        os.path.expanduser(FLAGS.acoustic_run_dir),
